...
| 图生图 | 比文生图慢一倍多：参考图也要变成 token 和画面一起算。开「参考图快速模式」参考图只按 1/4 像素参与，明显更快、发热少，但原图细节保留差一些 |
...
**模型常驻**：两个引擎都以后台服务方式运行（sd.cpp 用自带的 `sd-server`），模型加载一次后留在内存里，之后每张图直接开算。顶栏的「加载模型」按钮可以提前把模型读进内存（sd.cpp 会顺手出一张 256 的小图，因为它的权重要用到时才真正加载）；不用的时候点「释放内存」，模型占的内存（sd.cpp 约 5 GB，MLX 约 8–10 GB）就还给系统。打开工具时默认不加载；不点加载直接生成也可以，只是第一张要等加载。换模型、换引擎、切换省内存模式会自动重启引擎；取消生成也会重启引擎（它不支持中途打断）。
