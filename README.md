...
**模型常驻**：两个引擎都以后台服务方式运行（sd.cpp 用自带的 `sd-server`），模型只加载一次，之后每张图直接开算。打开工具时会在后台预载当前选中的模型（sd.cpp 会顺手出一张 256 的小图把权重拉进内存），界面状态栏显示「模型已就绪」即可。代价是工具开着时模型一直占着内存（sd.cpp 约 5 GB，MLX 约 8–10 GB），退出工具即释放。换模型、换引擎、切换省内存模式会自动重启引擎；取消生成也会重启引擎（它不支持中途打断），随后在后台重新预载。
